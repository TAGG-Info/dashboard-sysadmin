  ...
  document.getElementById('panelGeneral').innerHTML = `<form id="formGeneral" onsubmit="saveGeneral(event)"><div class="form-grid">
    ${GENERAL_SCHEMA.map(s => `
      ...
      ${s.fields.map(([field, label, type]) => {
        if (type === 'pj') {
          const pjFiles = getPJFilesForField(c.NoControleur, field);
          const fileLinks = pjFiles.map(pf => `<a class="pj-inline" href="#" onclick="openPJByDriveId('${pf.driveItemId}');return false;"><span class="pj-icon">&#128206;</span><span class="pj-name">${esc(pf.name)}</span></a>`).join('');
          ...
          return `<div class="form-field"><label>${label}</label><div class="pj-edit-wrap">${fileLinks || '<span class="pj-empty-inline">Aucune PJ</span>'}${uploadBtn}</div></div>`;
        }
        const raw = c[field] ?? '';
